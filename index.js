...
__الإقتراح__ :  \`\`\`${args.join(" ")}\`\`\`**`)
           client.channels.get("473966699629117453").send(embed)
...
client.on("message", (message) => {
      if (message.author.bot) return;
    if (message.content.startsWith('!!delvoice')) {
        if (!message.member.hasPermission('MANAGE_CHANNELS')) return message.reply("تحتاج لخاصية MANAGE_CHANNELS ");

        let args = message.content.split(' ').slice(1);
        let channel = message.client.channels.find('name', args.join(' '));
        if (!channel) return message.reply('**مافي روم بهذا اسم -_-**').catch(console.error);
        channel.delete()
    }
});
...
.setFooter("-")  
 ...
 const embed = new Discord.RichEmbed()
    ...
    .setDescription(`**معلوامت الاعضاء ✨
...
let voiceMembers = message.guild.channels.get('اي دي الروم');
if(message.content.startsWith(prefix + "voice-online")) {
...
client.on('guildMemberAdd', member => {
    member.guild.channels.get('474921986548695040').setName(`عدد الاعضاء: ${member.guild.memberCount}`);
    let humans = member.guild.memberCount - member.guild.members.filter(m => m.user.bot).size
    member.guild.channels.get('474921998896857128').setName(`عدد الاشخاص: ${humans}`);
    let bots = member.guild.members.filter(m => m.user.bot).size
    member.guild.channels.get('474922016684769291').setName(`عدد البوتات: ${bots}`);
});
